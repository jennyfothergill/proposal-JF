...
comparing forcefields to DFT specifically seeing if force field can accurately represent hydrogen bonding, pi-stacking, and other non-bonded interactions.
dispersion forces are very important for DNA stabilization
this paper looks at small molecule dimers which are relelvant to biomolecules in the gas phase (e.g., adenine/thymine dna base pair)
the break apart the molecules based on what is the dominant interaction: h-bonded, dispersion, or mixed.
All FFs underestimated hydrogen bonding but overall structures/energies are not bad.

\cite{Paul1995a}

united atom model for alkane chain melts. terminal methyl has deeper well than internal methylene groups. compared to experiments and all-atom model. good agreement for phase behavior, chain conformations, x-ray scattering profiles. Large-scale dynamics like self-diffusion agree with experiment and are 30\% faster!
first intro paragraph really concise c: remind us about the difficulty in selecting a model and there are many competing model for a system as simple as n-alkanes.


\cite{Yang2006a}
reduced models include solvent in the effective parameters. Amber UA for proeins. groups hydrogens with carbons except on backbone. United atom model dates back to 1970s the UNICEPP ff. nonpolar hydrogens are implicit. efficiency gain is. system size can reduce by roughly half since many hydrogens in biological/organic systems. fewer nonbonded and internal dof. larger dynamics step sizes since small hydrogen mass requires a smaller time step. and hydrogen position is not usually helpful for analysis methods like x-ray crystallography. 
by representing polar hydrogens this model can gain the benefits of UA while minimizing some disadvantages like UA ff not representing h-bonding or pi-stacking.

\cite{Levitt1976}
protein cg model--backbone represented by 3 beads (alpha-C, O, N) one bead per sidechain. protein sequence dictates folding so simplifying to one bead per amino acid seems reasonable. challenges: too many atoms, proteins fold in water at RT (opposed to small molecule crystals which can be calculated in vacuum at low temp)

\cite{McCoy1998a}
backmapping from UA to AA. ugh maybe skip this. 

\cite{Yi2018a}
PTB7-Th:ITIC/SF-PDI4 PCEs, mobilities, 1D scattering

\cite{S.Gurney2019b}
PTB7-Th ITIC PCE scattering patterns

\cite{Swick2019a}
ITIC derivatives, dft, crystal structures

\cite{Hammond2011a}
PTB7:PC71BM scattering PCE

\cite{Qin2016}
P3HT, PDCBT:ITIC scattering PCE

\cite{Pan2018c}
ITIC: PBDB-T and PBQ-0F molecular dynamics and dft

\cite{Shi2017d}
4TIC, PTB7-Th PCE scattering

\cite{Wang2019c}
PBTZF4, ITIC-Th experimental PCE values, mobilities

\cite{Gao2020}
polymer solar cells with nonfullerene acceptors - analysing morphology formation of nonfullerene/polymer cells with effective polymer thermodynamic quantity: Flory-Huggins interaction parameter.
>1000 NFAs
they cite a material getting over 18\% PCE?? refs 13,14
good starting point is miscibility
(flory huggins is related to miscibility but takes into account size differences between polymer/solvent)
this metric is useful for narrowing down what might be a good solar cell

\cite{Lee2020}
solvent evaporation study using MD
explicit solvent is removed intermittently from far above the surface to mimic the open air
this is different than other simulations where a solvent molecule was removed at random
they want to see the effects of concentration gradients
They see that some solvent molecules are trapped in the film and many species move less (based on MSD) as the ismulation progresses
